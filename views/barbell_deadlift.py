...
if "deadlift_workouts" not in st.session_state:
    st.session_state.deadlift_workouts = []
...
if not st.session_state.training_mode:
    ...
    if st.session_state.deadlift_workouts:
        # Pandas Tabelle mit letzten Trainings
        df = pd.DataFrame(st.session_state.deadlift_workouts)
    ...
    suggested_weight = get_suggested_weight("Barbell Deadlift", data_df)
    ...
    with col1:
        if st.button("✅ Speichern", use_container_width=True):
            ...
            st.session_state.deadlift_workouts.append(local_workout)
